...
export default async function (component) {
  ...
  const listEl = component.querySelector(
    '[fs-youtubefeed-element="list"]'
  );
  const templateItem = component.querySelector(
    '[fs-youtubefeed-element="item"]'
  );
  const loadMoreBtn = component.querySelector(
    '[fs-youtubefeed-element="load-more"]'
  );
  ...
  function renderHeader(channel) {
    ...
    const imgEl = component.querySelector(
      '[fs-youtubefeed-element="profile-image"]'
    );
    const nameEl = component.querySelector(
      '[fs-youtubefeed-element="profile-name"]'
    );
    const handleEl = component.querySelector(
      '[fs-youtubefeed-element="profile-handle"]'
    );
    const videoCountEl = component.querySelector(
      '[fs-youtubefeed-element="video-count"]'
    );
    const viewCountEl = component.querySelector(
      '[fs-youtubefeed-element="view-count"]'
    );
    const subCountEl = component.querySelector(
      '[fs-youtubefeed-element="subscriber-count"]'
    );
    const subscribeLinkEl = component.querySelector(
      '[fs-youtubefeed-element="subscribe-button"]'
    );
  ...
  function fillSingleItem(itemEl, video) {
    const thumbImg = itemEl.querySelector(
      '[fs-youtubefeed-element="thumbnail"]'
    );
    const thumbWrapper = itemEl.querySelector(
      '[fs-youtubefeed-element="thumbnail-wrapper"]'
    );
    const titleEl = itemEl.querySelector(
      '[fs-youtubefeed-element="title"]'
    );
    const descEl = itemEl.querySelector(
      '[fs-youtubefeed-element="description"]'
    );
    const dateEl = itemEl.querySelector(
      '[fs-youtubefeed-element="published-date"]'
    );
    const viewCountEl = itemEl.querySelector(
      '[fs-youtubefeed-element="view-count"]'
    );
    const likeCountEl = itemEl.querySelector(
      '[fs-youtubefeed-element="like-count"]'
    );
    const commentCountEl = itemEl.querySelector(
      '[fs-youtubefeed-element="comment-count"]'
    );
    const favouriteCountEl = itemEl.querySelector(
      '[fs-youtubefeed-element="favourite-count"]'
    );
    ...
    if (viewCountEl)
      viewCountEl.textContent = formatNumber(video.viewCount || "0");
    if (likeCountEl)
      likeCountEl.textContent = formatNumber(video.likeCount || "0");
    if (commentCountEl)
