...
class XYChart extends SerialChart {
  constructor() {
    ...
    Object.defineProperty(this, 'topAxesContainer', {
      ...
      value: this.chartContainer.children.push(
        Container.new(this._root, { width: p100, layout: this._root.verticalLayout })
      ),
    })
    /**
    ...
    Object.defineProperty(this, 'yAxesAndPlotContainer', {
      ...
      writable: true,
      value: this.chartContainer.children.push(
        Container.new(this._root, { width: p100, height: p100, layout: this._root.horizontalLayout })
      ),
    })
    /**
    ...
    Object.defineProperty(this, 'leftAxesContainer', {
      ...
      value: this.yAxesAndPlotContainer.children.push(
        Container.new(this._root, { height: p100, layout: this._root.horizontalLayout })
      ),
    })
    /**
    ...
    Object.defineProperty(this, 'plotsContainer', {
      ...
      writable: true,
      value: this.yAxesAndPlotContainer.children.push(
        Container.new(this._root, { width: p100, height: p100, maskContent: false })
      ),
    })
    /**
    ...
    Object.defineProperty(this, 'gridContainer', {
      ...
      value: this.plotContainer.children.push(
        Container.new(this._root, { width: p100, height: p100, isMeasured: false })
      ),
    })
    /**
  ...
  _afterNew() {
    ...
    this._disposers.push(
      plotContainer.events.on('pointerdown', (event) => {
        this._handlePlotDown(event)
      })
    )
    this._disposers.push(
      plotContainer.events.on('globalpointerup', (event) => {
        this._handlePlotUp(event)
      })
    )
    this._disposers.push(
      plotContainer.events.on('globalpointermove', (event) => {
  ...
  _setUpTouch() {
    if (!this.plotContainer._display.cancelTouch) {
      this.plotContainer._display.cancelTouch =
        this.get('pinchZoomX') || this.get('pinchZoomY') || this.get('panX') || this.get('panY') ? true : false
    }
  }
  _maskGrid() {
      ...
      this.xAxes.each((axis) => {
        if (axis.get('zoomX')) {
          ...
          let newStart = Math.min(
            1 + maxDeviation,
            Math.max(-maxDeviation, start - wheelStep * (end - start) * shiftX * position)
          )
          let newEnd = Math.max(
            -maxDeviation,
            Math.min(1 + maxDeviation, end + wheelStep * (end - start) * shiftX * (1 - position))
          )
    ...
    if ((wheelY === 'zoomX' || wheelY === 'zoomXY') && shiftY != 0) {
      this.xAxes.each((axis) => {
        if (axis.get('zoomX')) {
          ...
          let newStart = Math.min(
            1 + maxDeviation,
            Math.max(-maxDeviation, start - wheelStep * (end - start) * shiftY * position)
          )
          let newEnd = Math.max(
            -maxDeviation,
            Math.min(1 + maxDeviation, end + wheelStep * (end - start) * shiftY * (1 - position))
          )
          if (newStart == start && newEnd == end) {
            prevent = false
          }
  ...
  _handleSetWheel() {
    ...
    if (wheelX !== 'none' || wheelY !== 'none') {
      this._wheelDp = plotContainer.events.on('wheel', (event) => {
        const wheelEvent = event.originalEvent
        if (
          (wheelX !== 'none' && Math.abs(wheelEvent.deltaX) != 0) ||
          (wheelY !== 'none' && Math.abs(wheelEvent.deltaY) != 0)
        ) {
...
class XYSeries extends Series {
  ...
  _fixPosition() {
    ...
    this.set(
      'x',
      xAxis.x() - relativeToValue(xAxis.get('centerX', 0), xAxis.width()) - xAxis.parent.get('paddingLeft', 0)
    )
    this.set(
      'y',
      yAxis.y() - relativeToValue(yAxis.get('centerY', 0), yAxis.height()) - yAxis.parent.get('paddingTop', 0)
    )
  ...
  showDataItem(dataItem, duration) {
    ...
    return __awaiter(this, void 0, void 0, function* () {
      ...
      each$1(this._valueFields, (key) => {
        promises.push(
          dataItem
            .animate({ key: key + 'Working', to: dataItem.get(key), duration: duration, easing: easing })
            .waitForStop()
        )
      })
      yield Promise.all(promises)
      ...
      if (baseAxis === xAxis || !baseAxis) {
        each$1(this._valueYFields, (key) => {
          ...
          if (value != null) {
            promises.push(
              dataItem
                .animate({ key: key + 'Working', to: baseValue, duration: duration, easing: easing })
                .waitForStop()
            )
          }
        })
    ...
    if (tooltip) {
      if (!this.isHidden() && this.get('visible')) {
        ...
        if (dataItem) {
          ...
          const vcx = this.get('vcx', 1)
          const vcy = this.get('vcy', 1)
          const xPos = xAxis.getDataItemPositionX(
            dataItem,
            this._tooltipFieldX,
            this._aLocationX0 + (this._aLocationX1 - this._aLocationX0) * itemLocationX,
            vcx,
            exactLocationX
          )
          const yPos = yAxis.getDataItemPositionY(
            dataItem,
            this._tooltipFieldY,
            this._aLocationY0 + (this._aLocationY1 - this._aLocationY0) * itemLocationY,
            vcy,
            exactLocationY
          )
  ...
  constructor() {
    ...
    Object.defineProperty(this, 'bulletsContainer', {
      ...
      value: this.children.push(
        Container.new(this._root, { isMeasured: false, width: p100, height: p100, position: 'absolute' })
      ),
    })
    /**
  ...
  zoom(start, end, duration, priority) {
    if (this.get('zoomable', true)) {
      ...
      if (this.get('start') !== start || this.get('end') != end) {
        ...
        if (
          ((sAnimation && sAnimation.playing && sAnimation.to == start) || this.get('start') == start) &&
          ((eAnimation && eAnimation.playing && eAnimation.to == end) || this.get('end') == end)
        ) {
          return
        }
        if (duration > 0) {
...
class ValueAxis extends Axis {
  ...
  _handleSizeDirty() {
    if (this._sizeDirty && !this._dseHandled) {
      ...
      if (
        this.getPrivate('selectionMinFinal') != this.getPrivate('selectionMin') ||
        this.getPrivate('selectionMaxFinal') != this.getPrivate('selectionMax')
      ) {
        this._dirtySelectionExtremes = false
      }
    }
  ...
  positionToValue(position) {
    ...
    if (!this.get('logarithmic')) {
      ...
      return Math.pow(
        Math.E,
        (position * (Math.log(max) * Math.LOG10E - Math.log(min) * Math.LOG10E) + Math.log(min) * Math.LOG10E) /
          Math.LOG10E
      )
    }
  }
  ...
  getDataItemCoordinateX(dataItem, field, _cellLocation, axisLocation) {
    return this._settings.renderer.positionToCoordinate(
      this.getDataItemPositionX(dataItem, field, _cellLocation, axisLocation)
    )
  }
  /**
   * @ignore
  ...
  getDataItemCoordinateY(dataItem, field, _cellLocation, axisLocation) {
    return this._settings.renderer.positionToCoordinate(
      this.getDataItemPositionY(dataItem, field, _cellLocation, axisLocation)
    )
  }
  /**
   * @ignore
    ...
    if (isNumber(min) && isNumber(max)) {
      ...
      if (syncWithAxis) {
        minMaxStep = this._syncAxes(
          ...
          selectionMax,
          minMaxStep.step,
          syncWithAxis.getPrivate('selectionMinFinal', syncWithAxis.getPrivate('minFinal', 0)),
          syncWithAxis.getPrivate('selectionMaxFinal', syncWithAxis.getPrivate('maxFinal', 1)),
          syncWithAxis.getPrivate('selectionStepFinal', syncWithAxis.getPrivate('step', 1))
        )
  ...
  _getMinMax() {
    ...
    if (syncWithAxis) {
      minMaxStep = this._syncAxes(
        ...
        max,
        minMaxStep.step,
        syncWithAxis.getPrivate('minFinal', syncWithAxis.getPrivate('min', 0)),
        syncWithAxis.getPrivate('maxFinal', syncWithAxis.getPrivate('max', 1)),
    ...
    this.setPrivateRaw(
      'maxZoomFactor',
      Math.max(1, Math.ceil(((max - min) / minDiff) * this.get('maxZoomFactor', 100)))
    )
    this._fixZoomFactor()
    if (this.get('logarithmic')) {
...
class CategoryAxis extends Axis {
  ...
  getDataItemCoordinateX(dataItem, field, cellLocation, _axisLocation) {
    return this._settings.renderer.positionToCoordinate(
      this.getDataItemPositionX(dataItem, field, cellLocation, _axisLocation)
    )
  }
  /**
   * @ignore
  ...
  getDataItemCoordinateY(dataItem, field, cellLocation, _axisLocation) {
    return this._settings.renderer.positionToCoordinate(
      this.getDataItemPositionY(dataItem, field, cellLocation, _axisLocation)
    )
  }
  /**
...
class AxisRenderer extends Graphics {
  ...
  _afterNew() {
    ...
    if (thumb) {
      this._disposers.push(
        thumb.events.on('pointerdown', (event) => {
          this._handleThumbDown(event)
        })
      )
      this._disposers.push(
        thumb.events.on('globalpointerup', (event) => {
          this._handleThumbUp(event)
        })
      )
      this._disposers.push(
        thumb.events.on('globalpointermove', (event) => {
  ...
  _handleThumbMove(event) {
    ...
    if (downPoint) {
      ...
      const extra =
        ((this._getPan(point, downPoint) * Math.min(1, downEnd - downStart)) / 2) * this.get('panSensitivity', 1)
  ...
  constructor() {
    ...
    Object.defineProperty(this, 'thumb', {
      ...
      value: Rectangle.new(this._root, {
        width: p100,
        isMeasured: false,
        themeTags: ['axis', 'x', 'thumb', 'zoomgrip'],
      }),
    })
  }
  _afterNew() {
  ...
  constructor() {
    ...
    Object.defineProperty(this, 'thumb', {
      ...
      value: Rectangle.new(this._root, {
        height: p100,
        isMeasured: false,
        themeTags: ['axis', 'y', 'thumb', 'zoomgrip'],
      }),
    })
  }
  _afterNew() {
...
class ColumnSeries extends BaseColumnSeries {
  ...
  _processAxisRange(axisRange) {
    ...
    axisRange.columns = new ListTemplate(Template.new({}), () =>
      RoundedRectangle._new(
        this._root,
        {
          position: 'absolute',
...
//# sourceMappingURL=graphics-bars-DNK5Hpkc.js.map
