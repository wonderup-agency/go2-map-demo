...
function initTabs(component) {
  ...
  for (const root of roots) {
    ...
    wrappers.forEach((wrapper, wIdx) => {
      ...
      const recomputeHeight = () => { if (isDesktop()) gsapWithCSS.set(wrapper, { height: inner.offsetHeight }); };
      ...
      function applyMode() {
        if (isDesktop()) {
          ...
          gsapWithCSS.set(wrapper, { height: inner.offsetHeight });
          debug.note('mode: desktop');
        } else {
          wrapper.classList.add('tabs--mobile-pe-none');
          setVisualHitTestInline(wrapper); // keep taps on content only
      ...
      function closeDetails(scope, tlOr = gsapWithCSS) {
        getDetailsEls(scope).forEach((el) =>
          tlOr.to ? tlOr.to(el, { height: 0, duration: 0.25 }) : gsapWithCSS.set(el, { height: 0 })
        );
      }
      function openDetailsForMeasure(scope) {
        const restores = [];
        getDetailsEls(scope).forEach((el) => { restores.push(snapshotInlineStyle(el)); gsapWithCSS.set(el, { height: 'auto' }); });
        return () => restores.forEach((r) => r())
      }
      function hideAllVisualsExcept(keep) {
        visualItems.forEach((el) => { if (el !== keep) { el.classList.remove('active'); gsapWithCSS.set(el, { autoAlpha: 0, xPercent: 3 }); } });
      }
      ...
      function openTab(index) {
        ...
        if (isDesktop()) {
          ...
          restore();
          getDetailsEls(incomingContent).forEach((el) => gsapWithCSS.set(el, { height: 0 }));
          if (targetH > 0) gsapWithCSS.to(wrapper, { height: targetH, duration: 0.4, ease: 'power3.out' });
        }
        ...
        const tl = gsapWithCSS.timeline({
          ...
          onComplete: () => {
            ...
            isAnimating   = false;
            if (isDesktop()) gsapWithCSS.delayedCall(0, recomputeHeight);
      ...
      function collapseActive() {
        ...
        if (isDesktop()) tl.add(() => gsapWithCSS.to(wrapper, { height: inner.offsetHeight, duration: 0.3, ease: 'power2.out' }), 0);
      }
...
function toggleVisualPE(wrapper){
  ...
  if (on) setVisualHitTestInline(wrapper); else resetVisualHitTestInline(wrapper);
...
function hardReset(wrapper){
  ...
  gsapWithCSS.set(visuals,{autoAlpha:0,xPercent:3});
  gsapWithCSS.set(wrapper.querySelectorAll('[data-tabs="item-details"]'),{height:0});
}

export { initTabs as default };
//# sourceMappingURL=tabs-DH1ptt-u.js.map
